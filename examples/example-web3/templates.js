import { getNetwork } from '@holyheld/sdk';

export const getSpinnerHTML = () => '<img src="data:image/svg+xml;base64," width="24" height="24" />';

export const getSettingsHTML = (isTopupEnabled, minTopUpAmountInEUR, maxTopUpAmountInEUR) => `
  <dl>
    <dt>Functionality is available:</dt>
    <dd>${isTopupEnabled ? '<span style="color: green">Yes<span>' : '<span style="color: red">No<span>'}</dd>
    <dt>Minimum amount:</dt>
    <dd>${minTopUpAmountInEUR} EUR</dd>
    <dt>Maximum amount:</dt>
    <dd>${maxTopUpAmountInEUR} EUR</dd>
  </dl>
  <br>
  <label>
    $holytag:<br>
    <input type="text" value="SDKTEST"/>
  </label>
`;

export const getRadioItemHTML = (isFirst, address, network, name, balance, symbol) => `
  <div>
    <input type="radio" id="${address}${network}" name="drone" value="${address},${network}" ${isFirst ? 'checked' : ''}/>
    <label for="${address}${network}">
      ${name} |
      ${getNetwork(network).displayedName} |
      ${balance} ${symbol}
    </label>
  </div>
`;

export const getTokenInfoHTML = (name, address, network, balance, symbol) => `
  <dl>
    <dt>Name:</dt>
    <dd>${name}</dd>
    <dt>Address:</dt>
    <dd>${address}</dd>
    <dt>Network:</dt>
    <dd> ${getNetwork(network).displayedName}</dd>
    <dt>Balance:</dt>
    <dd>${balance} ${symbol}</dd>
  </dl>
  <br>
  <label>
    Token amount: <br>
    <input type="number" placeholder="5.00" /> ${symbol}
  </label>
`;

export const getDataHTML = (name, address, network, symbol, amount, amountInEUR, holytag) => `
  <dl>
    <dt>Name:</dt>
    <dd>${name}</dd>
    <dt>Address:</dt>
    <dd>${address}</dd>
    <dt>Network:</dt>
    <dd>${getNetwork(network).displayedName}</dd>
    <dt>Amount in ${symbol}:</dt>
    <dd>${amount} ${symbol}</dd>
    <dt>Amount in EUR:</dt>
    <dd>${amountInEUR} EUR</dd>
    <dt>$holytag:</dt>
    <dd>${holytag}</dd>
  </dl>
`;
